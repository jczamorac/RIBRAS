...
{
  posicao = G4ThreeVector(0.*mm, 0.*mm, -50.*cm);
  fposition = G4ThreeVector(0.*mm,0.*mm,-115.*cm);
  G4ParticleTable* particleTable = G4ParticleTable::GetParticleTable();
  G4IonTable* ionTable =  G4IonTable::GetIonTable();
  G4String particleName;

  particleGun->SetParticlePosition(posicao);
  // particleGun->SetParticlePosition(fposition);
  ...
  switch(j)
    {
    case 0:
      part = particleTable->FindParticle(particleName = "alpha");
      particleGun->SetParticleDefinition(part);
      particleGun->SetParticleEnergy(Energy);
      break;

    case 1:
      part = ionTable->GetIon(4,8,0.); //Be8
      particleGun->SetParticleDefinition(part);
      particleGun->SetParticleEnergy(Energy);
      break;

    case 2:
      part = particleTable->FindParticle(particleName = "He3");
      particleGun->SetParticleDefinition(part);
      particleGun->SetParticleEnergy(Energy);
      break;

    case 3:
      part = particleTable->FindParticle(particleName = "deuteron");
      particleGun->SetParticleDefinition(part);
      particleGun->SetParticleEnergy(Energy);
      break;

    case 4:
      part = ionTable->GetIon(4,7,0.); //Be7
      particleGun->SetParticleDefinition(part);
      particleGun->SetParticleEnergy(Energy);
      break;

    case 5:
      part = particleTable->FindParticle(particleName = "neutron");
      particleGun->SetParticleDefinition(part);
      particleGun->SetParticleEnergy(Energy);
      break;
    }
