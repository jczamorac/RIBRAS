 ...
 void DetectorConstruction::ConstructSetup(void)
{
 ...
 G4VPhysicalVolume* DetectorConstruction::Construct()
 {
  ...
  G4Box* alvo = new G4Box("alvo",7.5*CLHEP::cm,7.5*CLHEP::cm,0.5*CLHEP::mm);
  //G4VSolid* alvo = new G4Tubs("alvo", 0., raiointerno/2.0, 0.5*CLHEP::mm, 0., 360.*deg);

  G4LogicalVolume* alvolog = new G4LogicalVolume(alvo, G4NistManager::Instance()->FindOrBuildMaterial("G4_B"), "alvolog");

  G4VPhysicalVolume* alvophys = new G4PVPlacement(0, G4ThreeVector(), alvolog, "alvof", logmagnetico, false, 0, true);

  alvolog -> SetVisAttributes(new G4VisAttributes(yellow));
